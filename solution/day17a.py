...
class Crucible(object):
    ...
    @property
    def available_moves(self) -> Iterator[int]:
        last_move = self.previous_moves & 0b11

        yield last_move ^ 1
        yield last_move ^ 3

        if self.previous_moves % 21:
            yield last_move
    ...
    def make_start(start_pos:Vec2) -> list[tuple[int, 'Crucible']]:
        return [(0, Crucible(start_pos, dir)) for dir in (1, 2)]
...
def minimize_path(start_pos:Vec2, goal_pos:Vec2, cost_map:dict) -> int:
    ...
    while to_visit:
        ...
        for candidate in crucible.move_options:
            ...
            if candidate_distance >= distance_map[candidate]:
                continue
